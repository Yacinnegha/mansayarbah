...
function renderLoyQuestion(q) {
  document.getElementById("loy-question-text").textContent = q.prompt;
  ...
  q.options.forEach((opt, i) => {
    const btn = document.createElement("button");
    btn.className = "loy-option-btn";
    btn.type = "button";
    btn.innerHTML = `
      <span class="loy-option-letter">${["أ", "ب", "ج", "د"][i]}</span>
      <span class="loy-option-text">${escapeHtml(opt)}</span>
      <span class="loy-option-percent" style="display:none;">0%</span>
    `;
    btn.onclick = () => loyAnswer(i);
    grid.appendChild(btn);
  });
}
...
function loyAnswer(index) {
  ...
  // علّم الأزرار
  const buttons = document.querySelectorAll("#loy-options-grid .loy-option-btn");
  buttons.forEach((btn, i) => {
    btn.disabled = true;
  ...
  buttons.forEach((btn, i) => {
    const percentEl = btn.querySelector(".loy-option-percent");
