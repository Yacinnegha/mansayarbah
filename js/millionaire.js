...
function startGame(modeId) {
  ...
  // إخفاء نافذة الخسارة وأزرار ما بعد الخسارة من الجولة السابقة
  state.gameOver = false;
  const loseOverlay = document.getElementById("lose-modal-overlay");
  if (loseOverlay) loseOverlay.style.display = "none";
  const loseActions = document.getElementById("lose-actions-wrapper");
  if (loseActions) loseActions.style.display = "none";
...
/* ====== نافذة الخسارة ======
   بدل الانتقال الفوري لشاشة النهاية عند الخسارة، تبقى اللعبة في صفحة السؤال
   وتظهر نافذة فوقها تتيح: إغلاق النافذة لقراءة الإجابة الصحيحة وشرحها،
   أو بدء لعبة جديدة في نفس المود مباشرة. */
function loseGame(prize, reason) {
  stopTimer();
  state.gameOver = true;
  state.locked = true;
  state.awaitingNext = false;
  ...
  // كشف الإجابة الصحيحة (أخضر) وعرض الشرح أسفل الخيارات لقراءتهما بعد الإغلاق
  state.revealed = true;
  renderQuestion();

  // صوت الخسارة (مسار الإجابة الخاطئة يشغّله مسبقاً، ومسار نفاد الأرواح يشغّل صوت فقدان قلب)
  if (reason === "timeout") sounds.wrong();

  recordGameStats(false, prize, reason);
  showLoseModal(prize, reason);
}

function showLoseModal(prize, reason) {
  const icon = reason === "timeout" ? "⏰" : "💔";
  let title = "خسرت اللعبة!";
  if (reason === "timeout") title = "انتهى الوقت!";
  else if (reason === "ended") title = "نفدت الأرواح!";

  let subtitle = `إجابة خاطئة — توقفت عند السؤال ${state.currentIndex + 1}.`;
  if (reason === "timeout") subtitle = `انتهى الوقت قبل أن تجيب — توقفت عند السؤال ${state.currentIndex + 1}.`;
  else if (reason === "ended") subtitle = `نفدت جميع أرواحك — توقفت عند السؤال ${state.currentIndex + 1}.`;

  document.getElementById("lose-modal-icon").textContent = icon;
  document.getElementById("lose-modal-title").textContent = title;
  document.getElementById("lose-modal-subtitle").textContent = subtitle;
  document.getElementById("lose-modal-prize").textContent = prize.toLocaleString("en-US");
  document.getElementById("lose-modal-correct").textContent = state.totalCorrect;
  document.getElementById("lose-modal-streak").textContent = state.bestStreak;
  document.getElementById("lose-modal-overlay").style.display = "flex";
}

function closeLoseModal() {
  sounds.click();
  document.getElementById("lose-modal-overlay").style.display = "none";
  // إظهار زر بدء لعبة جديدة في صفحة السؤال نفسها
  const wrapper = document.getElementById("lose-actions-wrapper");
  if (wrapper) wrapper.style.display = "";
  // العودة لأعلى الصفحة لرؤية السؤال والإجابة الصحيحة
  window.scrollTo(0, 0);
}
...
function exitToMenu() {
  // بعد انتهاء اللعبة (خسارة) يُسمح بالخروج للقائمة دون قيد أو تأكيد
  const over = state.gameOver === true;
  if (state.locked && !over) return;
  if (!over && !confirm("هل تريد إنهاء اللعبة والعودة للقائمة الرئيسية؟")) return;
  sounds.click();
  stopTimer();
  showScreen("menu");
}
...
function endGame(won, prize, reason) {
  stopTimer();
  recordGameStats(won, prize, reason);
...
/* تسجيل نتيجة الجولة في الإحصائيات — مشترك بين شاشة النهاية ونافذة الخسارة */
function recordGameStats(won, prize, reason) {
