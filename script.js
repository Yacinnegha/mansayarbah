...
function loadFallbackApiKey() {
  try { state.fallbackApiKey = localStorage.getItem("millionaire_fallback_api_key") || ""; } catch {}
}

function saveFallbackApiKeyToStorage(key) {
  state.fallbackApiKey = key;
  try { localStorage.setItem("millionaire_fallback_api_key", key); } catch {}
}
...
function handleSaveModesApiKey() {
  ...
  // احفظ المفتاح الاحتياطي أيضاً إن وُجدت خانة
  const fallbackInput = document.getElementById("modes-fallback-api-key-input");
  if (fallbackInput) {
    saveFallbackApiKeyToStorage(fallbackInput.value.trim());
  }
...
/* ====== دالة موحدة لاستدعاء OpenRouter API مع دعم المفتاح الاحتياطي ======
   تجرب المفتاح الأساسي أولاً، وفي حال الفشل تنتقل للمفتاح الاحتياطي (إن وُجد).
   تُرجع { data, usedKey } عند النجاح، أو ترمي الخطأ الأخير عند الفشل الكامل.
   ملاحظة: OpenRouter قد يُرجع HTTP 200 مع خطأ في جسم الاستجابة (data.error)،
   نعامل هذا كفشل ونتقل للمفتاح التالي.
   
   ⚠️ مهم: لا نستخدم signal مباشرة مع fetch لكل محاولة، بل نُنشئ AbortController
   خاص بكل مفتاح (مع مهلة 30 ثانية) وندمجه مع signal الخارجي إن وُجد.
   هذا يسمح لكل مفتاح بمهلة كاملة، ويتجنب إلغاء المفتاح الاحتياطي
   عند انتهاء مهلة المفتاح الأساسي. */
async function callOpenRouterWithFallback({ body, signal, referer, title }) {
  const keys = [state.apiKey];
  if (state.fallbackApiKey && state.fallbackApiKey.trim().length > 0) {
    keys.push(state.fallbackApiKey);
  }
  let lastError = null;
  const PER_KEY_TIMEOUT = 30000; // 30 ثانية لكل مفتاح

  for (let i = 0; i < keys.length; i++) {
    const currentKey = keys[i];
    const keyLabel = i === 0 ? "الأساسي" : "الاحتياطي";

    // أنشئ AbortController خاص بهذه المحاولة مع مهلة خاصة
    const perKeyController = new AbortController();
    const perKeyTimer = setTimeout(() => perKeyController.abort(), PER_KEY_TIMEOUT);

    // ادمج signal الخارجي (إن وُجد) مع signal المفتاح الحالي
    // بحيث إلغاء الطلب الخارجي يُلغي المحاولة الحالية أيضاً
    let externalAbortHandler = null;
    if (signal) {
      if (signal.aborted) {
        // الطلب الخارجي مُلغى بالفعل — لا تحاول
        clearTimeout(perKeyTimer);
        lastError = new Error("الطلب مُلغى خارجياً");
        continue;
      }
      externalAbortHandler = () => perKeyController.abort();
    ...
    try {
      const res = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        method: "POST",
        signal: perKeyController.signal,
        headers: {
      ...
      if (!res.ok) {
        ...
        console.warn(`[API] فشل المفتاح ${keyLabel} (HTTP ${res.status}): ${errDetail.slice(0, 200)}`);
      ...
      // مثال: { "error": { "message": "Provider returned error", "code": 429 } }
      // أو محتوى choices يحتوي على نص خطأ مثل "Error: Provider returned error"
      if (data.error) {
        const errMsg = data.error.message || JSON.stringify(data.error);
        console.warn(`[API] المزود أرجع خطأ للمفتاح ${keyLabel}: ${errMsg}`);
      ...
      if (!content || (typeof content === "string" && content.trim().toLowerCase().startsWith("error:"))) {
        console.warn(`[API] استجابة فارغة أو خطأ في المحتوى للمفتاح ${keyLabel}: ${String(content).slice(0, 80)}`);
        lastError = new Error(content ? "خطأ في المحتوى: " + content.slice(0, 50) : "استجابة فارغة");
        continue; // جرّب المفتاح التالي
      }

      return { data, usedKey: keyLabel, keyIndex: i };
    } catch (err) {
      // ميّز بين انتهاء المهلة (per-key timeout) وإلغاء خارجي (signal)
      const isExternalAbort = signal && signal.aborted;
      if (isExternalAbort) {
        // الطلب أُلغي من الخارج — لا تتابع لمفتاح آخر
        console.warn(`[API] الطلب مُلغى خارجياً أثناء المفتاح ${keyLabel}`);
        lastError = err;
        throw err;
      }
      // انتهاء مهلة المفتاح أو خطأ آخر — جرّب المفتاح التالي
      console.warn(`[API] فشل المفتاح ${keyLabel}: ${err.message}`);
      lastError = err;
      continue;
    } finally {
      clearTimeout(perKeyTimer);
  ...
  // كل المفاتيح فشلت — ارمِ آخر خطأ
  throw lastError || new Error("لا يوجد مفتاح API متاح");
}
  ...
  try {
    const { data } = await callOpenRouterWithFallback({
      body: {
        ...
        temperature: 1.0,
        max_tokens: 600,  // زيادة من 400 لتقليل اقتطاع الاستجابة
      },
...
function renderSettings() {
  ...
  const fallbackInput = document.getElementById("fallback-api-key-input");
  ...
  const fallbackStatus = document.getElementById("fallback-api-status");
  if (fallbackStatus) {
    if (state.fallbackApiKey) {
      fallbackStatus.textContent = "✓ مفتاح احتياطي محفوظ — يُستخدم عند فشل المفتاح الأساسي";
      fallbackStatus.style.color = "#f5b800";
    } else {
      fallbackStatus.textContent = "لا يوجد مفتاح احتياطي — عند فشل الأساسي يُنتقل للبنك المحلي مباشرة";
      fallbackStatus.style.color = "rgba(255,255,255,0.5)";
    }
  }
}
...
function handleSaveApiKey() {
  ...
  saveApiKeyToStorage(key);
  // احفظ المفتاح الاحتياطي أيضاً إن وُجد
  const fallbackInput = document.getElementById("fallback-api-key-input");
  if (fallbackInput) {
    saveFallbackApiKeyToStorage(fallbackInput.value.trim());
  }
...
function handleClearApiKey() {
  ...
  const createFallbackInput = document.getElementById("create-fallback-api-key");
  if (createFallbackInput) createFallbackInput.value = "";
  renderSettings();
  sounds.click();
}

/* ====== حفظ/حذف المفتاح الاحتياطي ====== */
function handleSaveFallbackApiKey() {
  const input = document.getElementById("fallback-api-key-input");
  if (!input) return;
  const key = input.value.trim();
  // المفتاح الاحتياطي اختياري — يمكن أن يكون فارغاً
  saveFallbackApiKeyToStorage(key);
  renderSettings();
  const status = document.getElementById("fallback-api-status");
  if (status) {
    if (key) {
      status.textContent = "✓ تم حفظ المفتاح الاحتياطي بنجاح! سيُستخدم عند فشل المفتاح الأساسي.";
      status.style.color = "#f5b800";
    } else {
      status.textContent = "تم إزالة المفتاح الاحتياطي.";
      status.style.color = "rgba(255,255,255,0.5)";
    }
    sounds.correct();
    setTimeout(() => renderSettings(), 2500);
...
function handleClearFallbackApiKey() {
  saveFallbackApiKeyToStorage("");
  const input = document.getElementById("fallback-api-key-input");
...
function resetAllData() {
  ...
  localStorage.removeItem("millionaire_fallback_api_key");
  localStorage.removeItem("millionaire_sound");
  state.apiKey = "";
  state.fallbackApiKey = "";
...
async function createBattleRoom() {
  ...
  const fallbackApiKey = document.getElementById("create-fallback-api-key") ? document.getElementById("create-fallback-api-key").value.trim() : "";
  if (fallbackApiKey && !fallbackApiKey.startsWith("sk-or-")) {
    alert("مفتاح OpenRouter الاحتياطي يجب أن يبدأ بـ sk-or-");
    return;
  }
  ...
  try {
    const { data } = await callOpenRouterWithFallback({
      body: {
        ...
        temperature: 1.0,
        max_tokens: 500,  // زيادة من 400 لتقليل اقتطاع الاستجابة
      },
      // لا signal — callOpenRouterWithFallback يُدارة المهلة بنفسه
      referer: "https://who-wants-to-be-a-millionaire.local",
...
function updateLoyApiKeyCard() {
  ...
  if (state.apiKey && state.apiKey.trim().length > 0) {
    ...
    card.style.display = "none";
    if (hint) {
      const fallbackNote = state.fallbackApiKey ? " + مفتاح API احتياطي" : "";
      hint.textContent = "✓ مفتاح AI محفوظ" + fallbackNote + " — ستُولَّد الأسئلة بالذكاء الاصطناعي بشكل افتراضي مع بنك احتياطي.";
...
function handleSaveLoyApiKey() {
  ...
  saveApiKeyToStorage(key);
  // احفظ المفتاح الاحتياطي أيضاً إن وُجدت خانة
  const fallbackInput = document.getElementById("loy-fallback-api-key-input");
  if (fallbackInput) {
    saveFallbackApiKeyToStorage(fallbackInput.value.trim());
  }
  input.value = "";
